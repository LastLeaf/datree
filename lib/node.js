...
var createFromShapeWithBusy = function(shape, parent, busyNode, lock, cb){
    ...
    // construct core data
    // TODO add destroyed status to prevent changes after destroyed
...
var updateInAncestorChain = function(isFields, node, path, args, cb){
    ...
    if(node.$parent) {
        updateInAncestorChain(isFields, node.$parent, encodeURIComponent(node.$shape.parentPath) + (path ? '/' + path : ''), args, function(){
            if(this.interrupted) {
