    ...
    linkNodeToSource(node, function(){
        ...
        if(shape.type) {
            ...
            for(var k in shape.fields) (function(k){
                waitingChildren++;
                createFromShape(shape.fields[k], node, lock, function(childNode){
                    staticChildren[k] = childNode;
                    if(k.match(/^[_a-z][_a-z0-9]*$/i)) {
                        Object.defineProperty(node, k, {
                            get: function(){
                                if(childNode.$shape.type) return childNode.$cache;
                                return childNode;
                            },
                            enumerable: true
                        });
...
Node.prototype.getCachedValue = function(){
    return this.$cache;
};
Node.prototype.getDynamicChild = function(childName){
    return this.$dynamics[String(childName)];
};
Node.prototype.get = Node.prototype.getChild = function(childName){
...
var nodeUpdateFields = function(node, newOrder){
    ...
    node.$order = newOrder;
    // TODO diff and interrupt
    // TODO let dynamics enumerable through function and directly readable in object
...
var transferFields = function(destination, sourceChildNodes){
    ...
    sourceChildNodes.forEach(function(sourceChildNode, i){
        waiting++;
        // TODO do not create new if exists
