...
var createFromShapeWithBusy = function(shape, parent, busyNode, lock, cb){
    ...
    asyncForEach(shape.type ? {} : shape.fields, function(field, k, cb){
        ...
        staticChildren[k] = createFromShape(field, node, lock, function(n){
            staticChildren[k] = n;
            cb();
        });
    }, function(){
...
var createFromShape = function(shape, parent, lock, cb){
    ...
    if(shape.writable) {
        var busyNode = createFromShapeWithBusy(busyShape, null, undefined, Lock.create(), function(){});
        return createFromShapeWithBusy(shape, parent, busyNode, lock, cb);
    }
    return createFromShapeWithBusy(shape, parent, undefined, lock, cb);
};
var createTcb = function(cb, self){
    cb = cb || function(){};
    return function(arg0){
        setTimeout(function(){
            cb.call(self || global, arg0);
        }, 0);
    };
};
Node.create = function(shape, cb){
    var tcb = createTcb(cb);
    if(shape instanceof Shape) return createFromShape(shape, null, Lock.create(), tcb);
    return createFromShape(Shape._create(shape), null, Lock.create(), tcb);
};
var destroyNode = function(node){
    if(node.$destroyed) return;
    var k = '';
    for(k in node.$statics) {
...
var createDynamicChild = function(node, fieldName, def, cb){
    ...
    return createFromShape(shape, node, node.$lock, cb);
};
Node.prototype.createField = function(fieldName, def, cb){
    ...
    return createDynamicChild(this, fieldName, def, tcb);
};
Node.prototype.transform = function(def, cb){
    var tcb = createTcb(cb);
    var shape = Shape._create(def, this);
    return createFromShape(shape, null, Lock.create(), tcb);
};
...
var linkNodeToSource = function(node, cb){
    ...
    if(source) source.$destinations[destination.$id] = destination;
    requestTransferData(source, destination, cb);
};
