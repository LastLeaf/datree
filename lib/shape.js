...
var transformLink = function(shape, source){
    var sourceShape = Node.prototype._getShape.call(source);
    shape.link = source;
    if(sourceShape.type) {
        ...
        for(var k in sourceShape.fields) {
            ...
            var childSource = Node.prototype._getDescendant.call(source, k);
...
var normalizeFields = function(def, source, shape){
    var sourceShape = (source && Node.prototype._getShape.call(source)) || null;
...
Shape._create = function(def, parentSource, parentShape, parentPath){
    ...
    if((typeof(def.link || def.source) === 'string')) {
        if(parentSource) source = Node.prototype._getDescendant.call(parentSource, def.link || def.source);
