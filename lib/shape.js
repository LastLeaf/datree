    ...
    def = {
        ...
        fields/addFields: {
            ...
            'FIELD2': {
                ...
                // only for leaf nodes
                type: 'string' || 'number' || 'boolean' || 'ref' || 'json' || 'function'
                value: DEFAULT_VALUE
            }
        }
...
var matchValueWithType = function(value, type){
    ...
    if(type === 'ref') return value instanceof Node;
...
var getTypeByValue = function(value){
    if(value instanceof Node) return 'ref';
...
var transformLink = function(shape, source){
    ...
    if(sourceShape.type) {
        ...
        for(var k in sourceShape.fields) {
            ...
            var childSource = Node.prototype.getStaticChild.call(source, k);
...
var normalizeFields = function(def, shape, source){
...
Shape.create = function(def, parentShape, parentSource){
    // short def form
    ...
    if((typeof(def.link || def.source) === 'string')) {
        if(parentSource) source = Node.prototype.getStaticChild(parentSource, def.link || def.source);
    ...
    if(def.link) {
        ...
        if(def.type) {
            if(def.value === undefined) {
                ...
                else if(def.type === 'ref') shape.value = null;
        ...
        normalizeFields(def, shape, source);
    }
    ...
    if(shape.dynamic) {
        shape.syncFields = FuncArr.create(def.syncFields);
    } else if(shape.syncFields !== undefined) {
        throw new NodeError('"syncFields" option is not allowed to be defined in non-dynamic nodes.');
    }
