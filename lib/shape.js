...
var getTypeByValue = function(value){
    ...
    if(value instanceof FuncArr && typeof(value[0]) === 'function') return 'function';
    return 'json';
};
...
var transformLink = function(shape, source){
    ...
    if(sourceShape.type) {
        ...
        for(var k in sourceShape.fields) {
            ...
            var childSource = Object.getPrototypeOf(source)._getDescendant.call(source, k);
...
Shape._create = function(def, parentSource, parentShape, parentPath){
    ...
    if(typeof(def) === 'function') {
        def = { value: def };
    } else if(def.$shape instanceof Shape) {
        def = { link: def };
    ...
    if((typeof(def.link || def.source) === 'string')) {
        if(parentSource) source = Object.getPrototypeOf(parentSource)._getDescendant.call(parentSource, def.link || def.source);
        else throw new NodeError('Relative path is not supported without Node given in ancestors.');
    } else if(def.link || def.source) {
        source = def.link || def.source || null;
    }
    if(source && !(source.$shape instanceof Shape)) throw new NodeError('Source could only be another node.');
    ...
    if(shape.type === 'function') shape.writable = false;
