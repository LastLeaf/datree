...
var transformLink = function(shape, source){
    ...
    if(sourceShape.type) {
        ...
        for(var k in sourceShape.fields) {
            ...
            childShape.source = Node.prototype.getStaticChild.call(source, k);
...
Shape.create = function(def, parentShape){
    ...
    if((typeof(def.link || def.source) === 'string') && parentShape) {
        source = Node.prototype.getStaticChild(parentShape.source, def.link || def.source);
    ...
    // filters
    shape.request = FuncArr.create(shape.request);
    shape.update = FuncArr.create(shape.update);
    if(shape.dynamic) {
        shape.syncFields = FuncArr.create(shape.syncFields);
